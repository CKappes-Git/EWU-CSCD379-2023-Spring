  ...
  <v-container :class="[theme.global.name.value === 'dark' ? 'containerD' : 'containerL']">
    <div>
      <v-row>
        <v-text-field
          ...
          type="text"
          v-model="civName"
          @click="searchCiv(game, false)"
          @input="searchCiv(game, true)"
        ></v-text-field>
        <v-btn style="margin-top: 8px; background-color: green" @click="addCiv(game)"> + </v-btn>
        <v-btn style="margin-top: 8px; background-color: red" @click="deleteCiv(game)"> - </v-btn>
      </v-row>
      <div class="searchBox">
        <div v-for="(civ, index) in civResults" :key="index">
          <v-btn
            style="margin-bottom: 1px; margin-bottom: 1px; width: 100%"
            @click="setCiv(game, civ.civName)"
            >{{ civ.civName }}</v-btn
          >
        </div>
      </div>
    ...
    <div v-if="curCiv != null">
      <v-row>
        <v-text-field
          ...
          @input="search(game)"
        ></v-text-field>
        <v-btn style="margin-top: 8px; background-color: green" @click="addLeader(game)"> + </v-btn>
        <v-btn style="margin-top: 8px; background-color: red" @click="deleteLeader(game)">
          -
        </v-btn>
      </v-row>
      <div class="searchBox">
    ...
    <v-row v-if="curLeader != null">
      <v-col style="width: 100%" v-if="curLeader.civAttributes != null">
        ...
        <div style="width: 100%" v-for="(attribute, index) in curLeader.civAttributes" :key="index">
          <v-card style="margin-bottom: 15px">
            <v-btn
              ...
              @click="removeCivAttribute(attribute, index)"
              >X</v-btn
            >
        ...
        <v-btn @click="addCivAttribute"> + </v-btn>
      </v-col>
      <v-col v-if="curLeader.leaderAttributes != null">
        ...
        >
          <v-card style="margin-bottom: 15px">
            <v-btn
              style="color: red; margin-left: 90%; width: max-content"
              @click="removeLeaderAttribute(attribute, index)"
              >X</v-btn
            >
            <v-text-field label="Type" type="text" v-model="attribute.attributeType"></v-text-field>
            <v-text-field label="Name" type="text" v-model="attribute.abilityName"></v-text-field>
        ...
        <v-btn @click="addLeaderAttribute"> + </v-btn>
      </v-col>
    </v-row>
    <v-btn v-if="curLeader != null" @click="submit(game)" style="margin: 8px; width: 100%">
      Submit Changes
    </v-btn>
    <div style="margin: 20px" v-if="curLeader">
      <v-row>
        ...
        ></v-text-field>
        <v-btn style="margin-left: 3%; margin-top: 8px" @click="setBackground(game)">Set</v-btn>
      </v-row>
    </div>
  </v-container>
</template>
...
async function searchCiv(game = '', typed = false) {
  if (typed) {
    //if the user is typing on their keyboard, delete currently viewed data
    //this way clicking the search bar doesn't clear data accidentally
    curCiv.value = undefined
    curLeader.value = undefined
    leaderName.value = ''
    backgroundUrl.value = ''
  }
  let apiPath = `civilization/GetCivs?game=${game}&start=${civName.value}`
...
async function setCiv(game = '', name = '') {
  civName.value = name
  civResults.value = []
  let apiPath = `civilization/GetCivs?game=${game}&count=1&start=${civName.value}`
  Axios.get(apiPath).then((result) => {
    console.log(result.data)
...
async function search(game = '') {
  ...
  let apiPath = `civilization/GetLeaders?game=${game}&civName=${curCiv.value?.civName}&start=${leaderName.value}`
  Axios.get(apiPath).then((result) => {
    console.log(result.data)
...
async function setBackground(game = '') {
  let apiPath = `civilization/SetBackgroundUrl?game=${game}&civName=${curLeader.value?.civName}&url=${backgroundUrl.value}`
  Axios.post(apiPath)
    .then((result) => {
      console.log(result.data)
...
async function getUrl(game = '') {
  let apiPath = `civilization/GetBackgroundUrl?game=${game}&civName=${curLeader.value?.civName}`
  Axios.get(apiPath).then((result) => {
    console.log(result.data)
    backgroundUrl.value = result.data
  })
...
async function addLeader(game = '') {
  let apiPath = `civilization/AddLeader?game=${game}&civName=${curCiv.value?.civName}&leaderName=${leaderName.value}`
  Axios.post(apiPath).then((result) => {
    setLeader(game, result.data.name)
  })
}

async function addCiv(game = '') {
  let apiPath = `civilization/AddCiv?game=${game}&civName=${civName.value}`
  Axios.post(apiPath).then((result) => {
    setCiv(game, result.data.civName)
  })
}

async function deleteCiv(game = '') {
  const answer = window.confirm(
    'You are about to delete a civ, all leaders of that civ, and all attributes of both.  Are you sure you want to do this?'
  )
  if (answer) {
    const doubleCheck = window.confirm('This cannot be undone (easily), are you sure?')
    if (doubleCheck) {
      let apiPath = `civilization/DeleteCiv?game=${game}&civName=${civName.value}`
...
async function deleteLeader(game = '') {
  const answer = window.confirm(
    'You are about to delete a leader and all attributes.  Are you sure you want to do this?'
  )
  if (answer) {
    const doubleCheck = window.confirm('This cannot be undone (easily), are you sure?')
    if (doubleCheck) {
      let apiPath = `civilization/DeleteLeader?game=${game}&leaderName=${leaderName.value}`
      Axios.post(apiPath).then(() => {
        leaderName.value = ''
        curLeader.value = undefined
        backgroundUrl.value = ''
...
async function submit(game = '') {
  ...
  Axios.post(`/civilization/AddAttributes?game=${game}`, curLeader.value)
    .then((result) => {
      curLeader.value = result.data
    })
    .catch((error) => {
