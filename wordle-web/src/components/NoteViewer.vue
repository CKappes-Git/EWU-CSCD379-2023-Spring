<template>
  <v-container
    :class="[theme.global.name.value === 'dark' ? 'containerD' : 'containerL']"
  ...
  >
    <div v-if="SignInService.instance.isSignedIn">
      ...
      <div class="searchBox">
        <div v-for="(leader, index) in searchResults" :key="index">
          <v-btn
            ...
            @click="setData(game, leader.name)"
            >{{ leader.name }}</v-btn
          >
        </div>
      </div>
      <div style="margin-top: 5px" v-if="curLeader">
        ...
        <div v-else>
          <v-card-title> You have no notes, add them in edit mode </v-card-title>
        </div>
        <div v-if="curNote">
          <v-row v-if="editMode">
        ...
        <div v-else>
          <v-card-title v-if="notes.length"> You have not selected a note </v-card-title>
        </div>
      </div>
      <div v-else>
        <v-card-title> You have not selected a leader </v-card-title>
      </div>
    </div>
  </v-container>
</template>
...
async function submitNote() {
  if (curNote != undefined && curNote.value?.leaderID) {
    ...
    Axios.post('/civilization/SetLeaderNote', sendNote)
      .then((result) => {
...
async function deleteNote() {
  ...
  if (answer) {
    console.log(curNote.value?.leaderNoteID)
    Axios.post(`/civilization/DeleteLeaderNote?leaderNoteId=${curNote.value?.leaderNoteID}`)
      .then(() => {
